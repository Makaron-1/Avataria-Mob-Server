...
class Avatar(Module):
    ...
    async def buy_clothes(self, command, clothes, ctp, client):
        ...
        for item in clothes:
            ...
            if clid:
                ...
                name = cloth
            if name and name not in self.server.clothes[gender]:
                return
            if 'canBuy' in self.server.clothes[gender][name]:
                if int(self.server.clothes[gender][name]['canBuy']) != 1:
                    return
            elif 'vipOnly' in self.server.clothes[gender][name]:
                if int(self.server.clothes[gender][name]['vipOnly']) == 0:
            ...
            for res in ['gold', 'silver', 'rating']:
                if res in self.server.clothes[gender][name]:
                    if res == 'gold':
                        gold += int(self.server.clothes[gender][name][res])
                    elif res == 'silver':
                        silver += int(self.server.clothes[gender][name][res])
                    elif res == 'rating':
                        rating += int(self.server.clothes[gender][name][res])
            to_buy.append(name)
        if not to_buy or resurse['res']['gld'] < gold or\
                resurse['res']['slvr'] < silver:
            return
    ...
    async def wear_cloth(self, msg, client):
        ...
        for cloth in clths:
            if "clid" in cloth:
                ...
                tmp = cloth["tpid"]
            await Location(self.server, client).change_wearing(tmp, True)
        inv = await Location(self.server, client)._get_inventory()
        clths = await Location(self.server, client).get_clothes(client.uid, type_=2)
        ccltn = await Location(self.server, client).get_clothes(client.uid, type_=3)
        return await client.send({
    ...
    async def create_avatar(self, client):
        ...
        await self.server.redis.set(f"uid:{client.uid}:gld", 9999999999)
        await self.server.redis.set(f"uid:{client.uid}:slvr", 9999999999)
        await self.server.redis.set(f"uid:{client.uid}:enrg", 135)
        await self.server.redis.set(f"uid:{client.uid}:hrt", 145*6)
        await self.server.redis.set(f"uid:{client.uid}:exp", get_exp(level))
        await self.server.redis.sadd(f"rooms:{client.uid}", "livingroom")
        await self.server.redis.rpush(f"rooms:{client.uid}:livingroom", "#GromovRoom", 1)
        for i in range(1, 6):
