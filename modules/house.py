...
class House(Module):
    ...
    async def close_house(self, msg, client):
        ...
        if msg['data']['lt'] not in prises:
            return
        resurse = await Location(self.server, client).gen_plr()
        if resurse['res']['gld'] < prises[msg['data']['lt']]['gold'] or\
    ...
    async def check_outside_get_room(self, client):
        element = 0
        while element < 5:
            if not client.step_to_activate:
                element += 1
            else:
                return False
    ...
    async def room(self, msg, client):
        ...
        if subcommand == 'info':
            ...
            if client.room != msg['roomId'] and \
                    client.uid != client.room.split("_")[1]:
                return
            if msg['data']['tmid'] not in self.server.online:
                return
