...
class Billing(Module):
    ...
    async def buy_coint(self, msg, client):
        ...
        if msg['data']['pk'] not in packs:
            return
        res = await Location(self.server, client).gen_plr()
        if res['res']['gld'] < packs[msg['data']['pk']]['gld']:
            return
        ...
        return await client.send({'broadcast': False, 'text': f"Хэй, {packs[msg['data']['pk']]['coin']} коинов уже у тебя на счету!(AvaLand)"}, type_=36)

    async def buy_silver(self, msg, client):
        resurse = await Location(self.server, client).gen_plr()
        if resurse['res']['gld'] < msg['data']['gld']:
            return
        await self.server.redis.set(f"uid:{client.uid}:gld",
    ...
    async def buy_energy(self, msg, client):
        ...
        await Location(self.server, client).ntf_res()
        return await client.send({'broadcast': False, 'text': f"Вы получили {packs[msg['data']['pk']]['enrg']} энергии.(AvaLand)"}, type_=36)
    ...
    async def buy_gold(self, msg, client):
        ...
        await Location(self.server, client).ntf_res()
        return await client.send({'broadcast': False, 'text': f"На ваш счёт поступило {packs[msg['data']['pk']]} золотых монет.(AvaLand)"}, type_=36)
