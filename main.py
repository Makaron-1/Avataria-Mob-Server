...
class Server:
    ...
    async def technical_working(self, client):
        return await client.send({
                'zoneId': 'house',
                'error': {'code': 10, 'data': {'duration': 999999999,
                            'reason': "тест серверов avatarlife.", 'banTime': int(time.time()),
                            'reasonId': 4, 'unbanType': 'admin panel',
    ...
    async def _background(self):
        while True:
            ...
            self.logging(f"Players registrations: {await self.redis.get('uids')}")
